...
const CollectionDetails = async ({ params }: { params: Promise<{ collectionId: string }> }) => {
  ...
  return (
    <div className="px-10 py-5 flex flex-col items-center gap-8">
      ...
      />
      <p className="text-heading3-bold text-grey-1">{collectionDetails.title}</p>
      <p className="text-body-normal text-grey-1 text-center max-w-[900px]">{collectionDetails.description}</p>
      <div className="flex flex-wrap gap-16 justify-center">
        {collectionDetails.products && collectionDetails.products.length > 0 && collectionDetails.products.map((product: ProductType) => (
